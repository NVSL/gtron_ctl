...
function push_ssh_key_to_github() {
    ...
    curl -H "Content-type: application/json" -X POST -s -u $github_user -d "{\"title\":\"GadgtronDevelopment\",\"key\":\"$sshkey\"}" https://api.github.com/user/keys |save_log push_ssh_key
